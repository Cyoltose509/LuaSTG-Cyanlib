    ...
    if self.smear then
        if self.imgclass.used_ani then
            for i, s in ipairs(self.smear) do
                ...
                Core.Render.SimpleAnimation(self.img, self.ani + i, s.x, s.y, s.rot, s.hscale, s.vscale)
            end
        else
            for _, s in ipairs(self.smear) do
                Core.Render.SetSpriteState(self.img, mode, max(0, s.alpha), R, G, B)
                Core.Render.SimpleSprite(self.img, s.x, s.y, s.rot, s.hscale, s.vscale)
            end
        end
    end
...
function TypeBase:render()
    Core.Render.SetSpriteState(self._fog_img, self._blend, 255 * self.timer / self.fog_time, self._r, self._g, self._b)
    Core.Render.SimpleSprite(self._fog_img, self.x, self.y, self.rot, ((self.fog_time - self.timer) / self.fog_time * 3 + 1) * self.imgclass.size)
end

local FadeType = Object.Define(TypeBase)
M.FadeType = FadeType
function FadeType:render()
    Core.Render.SetSpriteState(self.img, self._blend, 255 * self.timer / self.fog_time, 255, 255, 255)
    Core.Render.SimpleSprite(self.img, self.x, self.y, self.rot, self.hscale * ((self.fog_time - self.timer) / self.fog_time + 1))
end
...
require("STG.Shots.Bullet.Resource")

-- ========================================
-- Default Bullet Type Registration
-- ========================================
-- Register basic bullet styles using cake community-creations sprite atlas.
-- Game modules should override with their own sprite registrations.

local function registerDefaultTypes()
    local R = M.Resource
    local default_colli = 4

    local types = {
        { "arrow_small",  0.75, 0.75, "stg:arrow_small",  false },
        { "arrow_medium", 1.0,  1.0,  "stg:arrow_mid",    true  },
        { "arrow_big",    1.5,  1.5,  "stg:arrow_big",    false },
        { "ball_small",   0.75, 0.75, "stg:ball_small",   false },
        { "ball_medium",  1.0,  1.0,  "stg:ball_mid",     true  },
        { "ball_big",     1.5,  1.5,  "stg:ball_big",     true  },
        { "knife",        1.2,  1.0,  "stg:knife",        true  },
        { "crystal",      1.0,  1.0,  "stg:square",       false },
        { "star",         0.8,  0.8,  "stg:star_small",   false },
        { "bubble",       1.5,  1.5,  "stg:ellipse",      true  },
        { "heart",        1.0,  1.0,  "stg:heart",        true  },
        { "note",         1.0,  1.0,  "stg:butterfly",    true  },
        { "light",        0.5,  0.5,  "stg:ball_light",   true  },
        { "bullet",       1.0,  1.0,  "stg:gun_bullet",   false },
    }
...
-- ========================================
-- Bullet Style Name Table (backward-compatible with legacy code)
-- ========================================

---@class STG.Shots.Bullet.Style
---Maps PascalCase style names to registered bullet type strings.
M.Style = {
    ArrowSmall   = "arrow_small",
    ArrowMedium  = "arrow_medium",
    ArrowBig     = "arrow_big",
    BallSmall    = "ball_small",
    BallMedium   = "ball_medium",
    BallBig      = "ball_big",
    Knife        = "knife",
    Crystal      = "crystal",
    Star         = "star",
