...
function Base:RenderDefaultLaserStyle(chains)
    ...
    if self.node > 0 then
        ...
        local tail = Core.Resource.Sprite.Get(data.node_img)
    ...
    for i = 1, #chains do
        ...
        if chain.length > 0 then
            ...
            if width > 0 then
                ...
                Core.Resource.Sprite.Get(data.img1)
                ...
                x = x + l1 * rot_cos
                y = y + l1 * rot_sin
                Core.Resource.Sprite.Get(data.img2)
                    :setState(blend, color)
                    :setPos(x, y)
                ...
                x = x + l2 * rot_cos
                y = y + l2 * rot_sin
                Core.Resource.Sprite.Get(data.img3)
                    :setState(blend, color)
                    :setPos(x, y)
            ...
            if self.head > 0 then
                local head = Core.Resource.Sprite.Get(data.head_img)
...
require("STG.Shots.Laser.Collider")
require("STG.Shots.Laser.Resource")
require("STG.Shots.Laser.Updater")