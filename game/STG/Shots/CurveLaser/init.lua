...
function Base:render()
    if self.node > 0 then
        ...
        Core.Render.SetSpriteState(img, self._blend, self._a, self._r, self._g, self._b)
        Core.Render.SimpleSprite(img, self.prex, self.prey, 18 * self.timer, n / 8)
        Core.Render.SimpleSprite(img, self.prex, self.prey, -18 * self.timer, n / 8)
    end
    render_laser(self)
...
require("STG.Shots.CurveLaser.Updater")
require("STG.Shots.CurveLaser.Resource")