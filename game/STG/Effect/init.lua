...
function hinter:render()
    if self.fade then
        Render.SetSpriteState(self.img, self._blend, self.t * 255, 255, 255, 255)
    else
        Render.SetSpriteState(self.img, self._blend, 255, 255, 255, 255)
    end
    Render.Utils.SimpleSprite(self.img, self.x, self.y, self.rot or 0, self.hscale or 1, self.vscale or 1)
end

function M.Hinter(img, size, x, y, t1, t2, fade)
    local self = Core.Object.Instantiate(hinter)
    self.img = img
    self.x = x
    self.y = y
    self.t1 = t1
...
function fade:render()
    Render.SetSpriteState(self.img, self.blend, self.color.a, self.color.r, self.color.g, self.color.b)
    Render.Utils.SimpleSprite(self.img, self.x, self.y, self.rot or 0, self.hscale or 1, self.vscale or 1)
end
function fade:frame()
...
function M.Fade(img, x, y, rot, vx, vy, omega, layer, blend, life_time, size1, size2, color1, color2)
    local self = Core.Object.Instantiate(fade)
    self.img = img
    self.x = x or 0
    self.y = y or 0
    self.rot = rot or 0
...
function enemy_death_break_ef:render()
    ...
    Render.SetSpriteState(self.img, Render.BlendMode.MulAdd, alpha, self.R, self.G, self.B)
    local h, v = 0.4 - t * 0.01, t * 0.2 + 0.7
    h = h * self.size
    v = v * self.size
    Render.Utils.SimpleSprite(img, x, y, rot + 15, h, v)
    Render.Utils.SimpleSprite(img, x, y, rot + 45, h, v)
    Render.Utils.SimpleSprite(img, x, y, rot + 75, h, v)
end
...
function enemy_death_simple_particle:render()
    Render.SetSpriteState(self.img, Render.BlendMode.MulAdd, 125 - 125 + 125 * sin(90 - min(1, self.timer / self.lifetime) * 90), self.r, self.g, self.b)
    Render.Utils.SimpleSprite(self.img, self.x, self.y, self.rot or 0, self.hscale or 1, self.vscale or 1)
end
...
---Burst of particles at a position (for player death, bomb, etc.)
---@param x number
---@param y number
---@param r number
---@param g number
...
function M.DeathBurst(x, y, r, g, b, count)
    count = count or 8
    for _ = 1, count do
        Object.New(enemy_death_simple_particle, x, y,
            rand:float(1.5, 4), rand:float(0, 360),
            rand:int(30, 60), rand:float(1, 2.5), r, g, b)
    end
end
