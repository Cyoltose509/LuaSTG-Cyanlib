...
local Behavior = require("STG.Enemy.Behavior")
...
function M:init(enemy)
    ...
    self.behavior = Behavior.Manager(enemy, self)
    self.view_data = {
...
function M:applyProfile(profile)
    ...
    self._score = profile.score or 0
end
...
function M:takeDamage(info, hit)
    ...
    -- Route through boss spell card if present
    local spell_mgr = self:getComponent("BossSpellManager")
    if spell_mgr and spell_mgr.spell_active then
        amount = spell_mgr:damageSpell(amount)
    end
...
function M:onDeath()
    ...
    local is_boss = self:getComponent("BossSpellManager") ~= nil
    STG.Event.Fire(STG.Event.Group.ENEMY_DEATH, self.enemy, self._score or 0)
    if is_boss then
        STG.Event.Fire(STG.Event.Group.BOSS_DEATH, self.enemy)
    end
...
---Add a behavior (replaces current queue).
---@param behavior STG.Enemy.Behavior.Base
function M:addBehavior(behavior)
    self.behavior:add(behavior)
end

---Queue a behavior after current ones finish.
---@param behavior STG.Enemy.Behavior.Base
function M:queueBehavior(behavior)
    self.behavior:queue(behavior)
end

---@return boolean
function M:hasActiveBehavior()
    return self.behavior:hasActive()
end
...
function M:addComponent(name, comp)
    ...
    if type(comp) == "table" then
        -- { ComponentName = constructor } pattern
        for cname, ctor in pairs(comp) do
            self.components[cname] = ctor(self.enemy, self)
        end
    else
