...
function M:death()
    -- Trigger multi-stage death animation via Death component
    local death_comp = self.system.component_sys:getComponent("Death")
    if death_comp then
        death_comp:startDeath()
    else
        -- Fallback: instant death
        local res_comp = self.system.component_sys:getComponent("Resource")
        if res_comp then
            local remaining = res_comp:loseLife()
            if remaining <= 0 then
                self.system:setPhase("dead")
            else
                self:resetContainers()
                self:setInvincible(self.invincible_default * 4)
            end
        end
    end
end

---Reset all containers to full.
function M:resetContainers()
    -- Remove trashed containers
    for i = #self.containers, 1, -1 do
        if self.containers[i].trashed then
            table.remove(self.containers, i)
        end
    end
    -- Refill remaining
    for _, ctn in ipairs(self.containers) do
        ctn.current = ctn.capacity
        ctn.trashed = false
    end
end
...
function M:damage(count)
    ...
    if not ctn then
        STG.SE.Play("pldead00")
        self:death()
        return
    end
    STG.SE.Play("damage00")
