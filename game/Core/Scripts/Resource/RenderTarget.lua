---@class Core.Resource.RenderTarget
local M = {}
Core.Resource.RenderTarget = M
M.__index = M
M.__type = "RenderTarget"

---@type table<string, Core.Resource.RenderTarget>
M.res = {}

function M.New(name, width, height, depth_buffer)
    if M.res[name] then
        M.Remove(name)
    end

    ---@type Core.Resource.RenderTarget
    ...
    if width then
        lstg.CreateRenderTarget(name, width, height, depth_buffer)
    else
...
function M.Remove(name)
    if M.res[name] then
        lstg.RemoveResource(M.res[name].res_pool, Core.Resource.ResType.RenderTarget, name)
        M.res[name] = nil
    end
end
...
---@return Core.Resource.RenderTarget
function M.Get(name)
    return M.res[name]
end
...
function M:getSize()
    self.width, self.height = lstg.GetTextureSize(self.name)
