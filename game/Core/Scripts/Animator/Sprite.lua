---@class Core.Animator.Sprite
---@author OLC
local M = {}
Core.Animator.Sprite = M
...
function M:setState(stateName)
    self.stateMachine:setState(stateName)
    return self
end
...
function M:registerState(name, callbacks)
    self.stateMachine:registerState(name, callbacks)
    return self
end
...
function M:render()
    ...
    local texture = Core.Resource.Texture.Get(imgFrame.texture or self.texture)
    if not texture then
        return
    end
    ...
    local z = 0.5
    texture:setUV1(x - w * cosr - h * sinr, y - w * sinr + h * cosr, z, tx, ty, color)
           :setUV2(x + w * cosr - h * sinr, y + w * sinr + h * cosr, z, tx + tw, ty, color)
           :setUV3(x + w * cosr + h * sinr, y + w * sinr - h * cosr, z, tx + tw, ty + th, color)
           :setUV4(x - w * cosr + h * sinr, y - w * sinr - h * cosr, z, tx, ty + th, color)
           :setBlend(blend):draw()
end
